...
public class Main extends Application {
	...
	public void start(Stage primaryStage) {
		try {
			...
			// VBox für die Labels und Eingabefelder für IP-Adresse, Port,
			...
			vbox_eingaben.getChildren().addAll(lb_Benutzername, tf_Benutzername, lb_Passwort, pf_Passwort, lb_IPAdresse,
					tf_IPAdresse, lb_Port, tf_Port);
			vbox_eingaben.setVisible(false);

			VBox vbox_Serverauswahl = new VBox(headerLabel, rb_liveDB, rb_testDB, rb_manuell, vbox_eingaben,
					bt_starten);
			...
			rb_liveDB.setOnAction(new EventHandler<ActionEvent>() {
				public void handle(ActionEvent e) {
					// Labels und Eingabefelder sichtbar machen, entsprechende
					// Felder deaktivieren und mit Standardtext füllen
			...
			bt_starten.setOnAction(new EventHandler<ActionEvent>() {
				public void handle(ActionEvent e) {
					...
					if (tf_Benutzername.getText().isEmpty() || tf_IPAdresse.getText().isEmpty()
							|| tf_Port.getText().isEmpty()) {
						Alert alarm = new Alert(Alert.AlertType.WARNING, "Alle Felder müssen ausgefüllt sein.",
								ButtonType.OK);
						alarm.setTitle("Fehler!");
						alarm.show();

					} else {
						// Verbindung zum Server mit den entsprechenden Eingaben
						// herstellen und in der Variable serverConnection
						// speichern
						serverConnection = DatabaseFunctions.connectToServer(
								"jdbc:mysql://" + tf_IPAdresse.getText() + ":" + tf_Port.getText(),
								tf_Benutzername.getText(), pf_Passwort.getText());

					}

					// Wenn eine Verbindung hergestellt wurde, öffne das nächste
					// Fenster
					if (serverConnection != null) {
						Alert al_ConnectionSuccessful = new Alert(AlertType.INFORMATION,
								"Die Verbindung wurde hergestellt.", ButtonType.OK);
